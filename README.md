...
```
du-an-nguyen-lieu/
├── index.html              ← WEBSITE bán hàng công khai (nguyenlieuphache.com) — trang khách hàng xem,
│                              đặt hàng, xem sản phẩm. Ảnh đã nhúng sẵn bên trong.
├── noi-bo.html             ← Dashboard NỘI BỘ (không public trên menu) — tổng quan xu hướng,
│                              danh mục hàng, nhà cung cấp, kế hoạch 90 ngày, agent. Chỉ chủ shop dùng.
├── *.jpg                   ← 20 ảnh gốc chất lượng cao (dùng riêng để in ấn, đăng bài, chào hàng)
├── README.md               ← File này
...
- **Trang khách hàng (public):** `index.html` — đây là trang chạy tại **nguyenlieuphache.com**. Khách xem sản phẩm, giá tham khảo, bấm gọi/Zalo đặt hàng trực tiếp.
- **Dashboard nội bộ:** `noi-bo.html` — chỉ chủ shop dùng, không nằm trong menu công khai. Có 6 tab: Tổng quan · Xu hướng · Nguyên liệu (ảnh trực quan, lọc 🇻🇳/🌏/🔥 + bảng chi tiết) · Nhà cung cấp · Kế hoạch 90 ngày · Agent.

## 🤖 2. Dùng Agent có bộ nhớ (không cần code)

1. Mở phiên chat mới trên **Arena.ai** (hoặc AI nào có tìm kiếm web).
2. Dán toàn bộ `system-prompt.md`, rồi dán tiếp `memory.json`.
3. Ra lệnh, ví dụ:
...
python collector.py           # chạy — kết quả lưu vào news-YYYY-MM-DD.md
```

Tự động mỗi sáng 7h:
- **Windows:** Task Scheduler → Create Basic Task → Daily 07:00 → Start a program: `python` + arguments `collector.py` + start in: thư mục dự án.
- **Linux/Mac:** `crontab -e` thêm dòng: `0 7 * * * cd /duong/dan/du-an-nguyen-lieu && python collector.py`
